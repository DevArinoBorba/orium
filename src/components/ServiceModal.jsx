    ...
    >
      ...
      <div className="relative z-10 max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-3xl border border-primary/30 bg-surface p-6 sm:p-8 shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <button
          ...
          className="absolute right-5 top-5 flex h-10 w-10 items-center justify-center rounded-full border border-border bg-card/60 text-muted-foreground hover:text-foreground hover:bg-white/5 transition-colors focus-visible:ring-2 focus-visible:ring-primary focus:outline-none"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h3 id="service-modal-title" className="font-display text-2xl font-bold text-foreground">
              {service.title}
            </h3>
          </div>
        </div>

        <p id="service-modal-desc" className="mt-4 text-base leading-relaxed text-muted-foreground">
          {service.desc}
        </p>
