...
export function Testimonials() {
  return (
    <section id="depoimentos" className="relative py-28 border-t border-white/10 bg-[#08070d] overflow-hidden">
      {/* Background Lighting */}
          ...
          {testimonials.map((item, idx) => (
            <Reveal key={item.name} delay={idx * 100}>
              <SpotlightCard
                ...
                className="h-full flex flex-col justify-between border-white/10 hover:border-purple-500/50 group"
              >
                <div>
                  ...
                  {/* Quote Body */}
                  <p className="text-sm sm:text-base text-zinc-300 leading-relaxed italic relative">
                    <Quote className="h-5 w-5 inline text-purple-400/50 mr-2 -mt-1" />
                    "{item.text}"
                  </p>
                </div>
                ...
                <div className="mt-8 pt-5 border-t border-white/10 flex items-center gap-3.5">
                  ...
                  <div>
                    <h4 className="font-display text-sm font-bold text-white group-hover:text-purple-200 transition-colors">
                      {item.name}
                    </h4>
