...
export function Preloader({ onComplete }) {
  ...
  return (
    <div
      ref={containerRef}
      id="preloader"
      className="fixed inset-0 z-[100] flex flex-col justify-between bg-[#faf8fd] dark:bg-[#08070c] p-6 sm:p-10 text-zinc-950 dark:text-white overflow-hidden transition-colors"
      aria-hidden="true"
      style={{ willChange: 'transform' }}
    >
      {/* Top Bar: Brand & Subtitle */}
      <div className="flex items-center justify-between text-zinc-600 dark:text-zinc-400 text-xs sm:text-sm font-medium tracking-wide">
        <div className="flex items-center gap-2.5">
          <span className="h-2 w-2 rounded-full bg-purple-500 animate-pulse shadow-[0_0_10px_#a855f7]" />
          <span className="font-display font-bold tracking-wider text-zinc-950 dark:text-white text-sm sm:text-base">
            ORIUM DIGITAL
          </span>
        </div>
        <span className="text-[11px] sm:text-xs uppercase tracking-[0.25em] text-zinc-600 dark:text-zinc-400/80 font-semibold hidden xs:inline">
      ...
      <div className="my-auto py-8">
        <p
          ref={wordRef}
          className="font-display text-center text-[clamp(2.2rem,7.5vw,6.5rem)] font-light leading-[1.05] tracking-tight text-zinc-950 dark:text-white select-none"
        >
                ...
                <span key={cIdx} className="inline-block overflow-hidden align-top">
                  <span
                    className={`pre-char inline-block ${
                      wordObj.highlight
                        ? 'italic font-serif text-transparent bg-clip-text bg-gradient-to-r from-purple-600 via-violet-500 to-indigo-600 dark:from-purple-300 dark:via-violet-400 dark:to-indigo-300 font-normal pr-0.5'
      ...
      <div className="relative flex items-end justify-between pb-2 sm:pb-3">
        <div className="flex items-baseline select-none">
          <span
            ref={countRef}
            className="pre-count font-display text-[clamp(4.5rem,18vw,14rem)] font-light leading-[0.75] text-zinc-950 dark:text-white tracking-tighter tabular-nums"
          >
            0
          </span>
          <span className="text-purple-600 dark:text-purple-400 font-display text-2xl sm:text-4xl font-light ml-1 select-none">
            %
          </span>
        </div>
        <span className="text-xs sm:text-sm uppercase tracking-[0.25em] text-zinc-500 dark:text-zinc-400 font-medium pb-2 sm:pb-4 select-none">
